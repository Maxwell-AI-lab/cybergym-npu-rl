...
```
┌─────────── 中心服务: x86 (192.168.0.100) ──── 沙箱主机池: 可横向扩容 ───────────────────┐
│                                                                                        │
│  ┌─ OpenHands Runtime 容器池（跨主机，加机器即扩容 §12）───────────────────────────┐   │
│  │ [x86 本机 ×12~24] [沙箱主机B ×M] [沙箱主机C ...] ← sandbox_hosts.yaml 注册       │   │
│  │ 官方通用镜像 · 每任务/每轨迹一个实例                                              │   │
│  │ workspace: repo-vul.tar.gz + description.txt + README.md + submit.sh             │   │
│  │ agent 行为: 解压源码 → 读码分析 → 写 PoC → bash ./submit.sh PATH(可多次)          │   │
│  └──────────────┬─────────────────────────── HTTP 可达即可（无需集群/NPU）──────────┘   │
