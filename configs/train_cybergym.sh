...
ACTOR_CONFIG=(
    ...
    +actor_rollout_ref.actor.mindspeed.llm_kwargs.compress_ratios="[0,0,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4,128,4]"
...
PYTHONUNBUFFERED=1 python3 -c "import sys; sys.path.insert(0,'/data/z00666713/deepseek0715'); import register_dsv4" && PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
    ...
    "${REWARD_CONFIG[@]}" \
    "${TRAINER_CONFIG[@]}" \
    "$@" 2>&1 | stdbuf -oL tee /data/z00666713/deepseek0715/logs/cybergym_$(date +%Y%m%d_%H%M%S).log
