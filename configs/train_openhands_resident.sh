...
#!/bin/bash
# ============================================================
# T5 Offline-Replay Training: consume pre-collected OpenHands trajectories
#
# Uses verl RolloutSkip: generate_sequences() loads the dumped DataProto
# (built by prep_offline_dump.py from trajproxy trajectories) instead of
# calling vLLM. GRPO groups by task (uid=task_id), rewards embedded as
# rm_scores (final-submission verdicts from poc.db).
#
# Legacy multi-turn tool config kept for engine compatibility:
...
export CYBERGYM_LOOP_DUMP_DIR=/data/dataset/openhands_traj/rollout_dump/cybergym_openhands_offline_verl_gsm8k/GBS64_N1_in12288_out1024
export CYBERGYM_LOOP_GBS=64
...
export VERL_ROLLOUT_BUNDLE_CUSTOM_RESOURCES='{"rollout_node": 1}'
...
project_name="verl_gsm8k"  # must match dump dir {exp}_{proj}
exp_name="cybergym_openhands_offline"
...
CKPTS_DIR=/data/z00666713/deepseek0715/checkpoints_openhands_offline
TRAIN_FILE=/data/dataset/openhands_traj/batch2_min.parquet
TEST_FILE=/data/dataset/openhands_traj/batch2_min.parquet

# ---- 数据长度 (多轮模式需要更多空间) ----
max_prompt_length=12288              # 12K cap: v7 OOM root cause = 16K seq training memory
max_response_length=1024             # covers longest turn response 986

# ---- Batch ----
train_prompt_bsz=64
train_prompt_mini_bsz=64
n_resp_per_prompt=1                  # offline replay: samples already per-turn
...
DATA_CONFIG=(
    ...
    +data.custom_cls.path=/data/z00666713/deepseek0715/cybergym_integration/verl_integration/blocking_dataset.py
    +data.custom_cls.name=BlockingRLHFDataset
...
ACTOR_CONFIG=(
    ...
    +actor_rollout_ref.actor.mindspeed.llm_kwargs.global_batch_size=64
...
ROLLOUT_CONFIG=(
    ...
    actor_rollout_ref.rollout.skip.dump_dir=/data/dataset/openhands_traj/rollout_dump
...
PYTHONUNBUFFERED=1 python3 -c "import sys; sys.path.insert(0,'/data/z00666713/deepseek0715'); import register_dsv4" && PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
    ...
    "${AGENT_LOOP_CONFIG[@]}" \
    "${TRAINER_CONFIG[@]}" \
    "$@" 2>&1 | stdbuf -oL tee /data/z00666713/deepseek0715/logs/openhands_resident_$(date +%Y%m%d_%H%M%S).log