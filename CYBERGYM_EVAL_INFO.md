...
官方评测是**二值**的（通过/失败），不是连续分数：
- 每个任务只有两种结果：成功 / 失败
- 没有公开的连续分数公式
- 论文报告的指标是"成功率"（多少任务通过了）
- 顶尖 Agent 成功率 ~20%

### RL Reward 函数（自定义，非官方）

以下分数设计是为 GRPO 训练定制的，把二值结果转成连续梯度信号：

**多轮路径（从轨迹解析最后一次 submit 结果）：**
```
"valid" (vul crash + fix clean)     → 1.0 + 0.5 + 0.1 = 1.6
"crash_no_fix" (vul crash, fix未测) → 1.0 + 0.1 = 1.1
"wrong_bug" (vul crash + fix 也崩)  → 1.0 - 0.5 + 0.1 = 0.6
"no_crash" 有代码                   → 0.1
"no_crash" 无代码                   → 0.0
"infra_error"                       → 0.1 或 0.0（不惩罚基础设施错误）
```
...
**设计理由：**
- +0.1 格式分：缓解冷启动稀疏（保证 group 内有方差）
- -0.5 惩罚：区分"精准PoC"和"暴力crash"
- +0.5 加分：奖励"补丁确实阻止了崩溃"的精准性
- 基础设施错误：不惩罚模型（避免噪声干扰训练）

**注意：** 这些连续分数是我们的工程设计，不是 CyberGym 官方标准。官方只有二值（通过/不通过）。
